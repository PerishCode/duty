...
writes a PR review brief (typically to `.tmp/duty/reviews/<num>.md` as
...
Pnpm command list derived from touched packages (matches `duty view`
...
style applied to three different PR shapes. See `templates/examples/`:
...
`.tmp/duty/reviews/<num>.md` and are transient by design.
